...
import com.example.aoa.helpers.*
import com.example.aoa.receivers.CombinedServiceReceiver
import com.example.aoa.services.NotificationService

class AlwaysOn : com.example.aoa.actions.OffActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (prefs.get(P.SHOW_MUSIC_CONTROLS, P.SHOW_MUSIC_CONTROLS_DEFAULT)) {
            ...
            onActiveSessionsChangedListener =
                AlwaysOnOnActiveSessionsChangedListener(
                    viewHolder,
                    resources
                )
            try {
    ...
    override fun onStart() {
        ...
        if (prefs.get(P.POCKET_MODE, P.POCKET_MODE_DEFAULT)) sensorManager?.registerListener(sensorEventListener, sensorManager?.getDefaultSensor(Sensor.TYPE_PROXIMITY),
            SENSOR_DELAY_SLOW,
            SENSOR_DELAY_SLOW
        )
    }
